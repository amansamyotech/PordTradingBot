...
const FUTURES_API_BASE = "https://testnet.binancefuture.com";  // Testnet URL
const apiKey = "YOUR_TESTNET_FUTURES_API_KEY";  // Testnet API Key
const apiSecret = "YOUR_TESTNET_FUTURES_API_SECRET";  // Testnet API Secret
...
const getPrice = async (symbol) => {
  try {
    const res = await axios.get(`${FUTURES_API_BASE}/fapi/v1/ticker/price`, { params: { symbol } });
...
const distributeBalanceEqually = async () => {
  ...
  logToFile(`Distributed $${total.toFixed(2)} equally across coins.`, "BALANCE");
};
  ...
  try {
    if (!state.position && alloc > 0) {
      ...
      if (order && order.status === "FILLED") {
        ...
        state.lastTradeTime = Date.now();
        logToFile(`Sold ${symbol} @ ${sellPrice} | Profit: $${profit.toFixed(2)}`, "PROFIT");
      }
    }
  } catch (e) {
