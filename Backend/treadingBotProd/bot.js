...
const PROFIT_TARGET = 0.03; // Step 4: 2% profit target

let coinStates = {}; // Store state per symbol
let pendingOrders = {}; // Track pending orders to prevent duplicates
...
const getPrecisionMap = async () => {
  ...
  SYMBOLS.forEach((symbol) => {
    ...
    const stepSize = info.filters.find(f => f.filterType === "LOT_SIZE").stepSize;
...
// Fetch existing open positions for SYMBOLS and initialize coinStates
...
const waitForOrderFill = async (symbol, orderId, maxWaitTime = 30000) => {
  ...
  while (Date.now() - startTime < maxWaitTime) {
    ...
    log(`⏳ Waiting for ${symbol} order to fill... Status: ${orderStatus.status}`);
    ...
    for (const symbol of SYMBOLS) {
      ...
      if (!coinStates[symbol]) {
        ...
        if (currentBalance < TRADE_AMOUNT) {
          log(`⚠️ Insufficient balance for ${symbol} (need $${TRADE_AMOUNT}, have $${currentBalance.toFixed(2)})`);
          continue;
        }

        const quantity = parseFloat((TRADE_AMOUNT / currentPrice).toFixed(precision));
        ...
        if (buyOrder) {
          if (buyOrder.status === "FILLED") {
            ...
            coinStates[symbol] = {
              ...
              targetPrice: parseFloat(buyOrder.fills[0].price) * (1 + PROFIT_TARGET),
            };
            log(`✅ BOUGHT ${buyOrder.executedQty} ${symbol} @ ${buyOrder.fills[0].price} | Target: ${coinStates[symbol].targetPrice.toFixed(6)}`);
          } else {
            // Order placed but not filled immediately
            log(`⚠️ Order placed but not filled immediately for ${symbol} | OrderID: ${buyOrder.orderId}`);
            ...
            if (filledOrder) {
              coinStates[symbol] = {
                ...
                targetPrice: parseFloat(filledOrder.avgPrice) * (1 + PROFIT_TARGET),
              };
              log(`✅ CONFIRMED FILLED ${filledOrder.executedQty} ${symbol} @ ${filledOrder.avgPrice} | Target: ${coinStates[symbol].targetPrice.toFixed(6)}`);
            } else {
        ...
        const priceChangePercent = ((currentPrice - state.buyPrice) / state.buyPrice * 100).toFixed(2);

        log(`📊 ${symbol} | Buy: $${state.buyPrice.toFixed(6)} | Now: $${currentPrice.toFixed(6)} | Change: ${priceChangePercent}% | Target: $${state.targetPrice.toFixed(6)}`);
        ...
        if (profitPercent >= PROFIT_TARGET) {
          ...
          if (sellOrder) {
            if (sellOrder.status === "FILLED") {
              ...
              log(`🎯 SOLD ${state.quantity} ${symbol} @ $${sellPrice.toFixed(6)} | Profit: $${profit.toFixed(4)}`);

              delete coinStates[symbol];
              log(`🔄 ${symbol} cycle completed. Restarting from step 1...`);
            } else {
              // Wait for sell order to fill
              const filledSellOrder = await waitForOrderFill(symbol, sellOrder.orderId);
...
const initStateFromBinance = async () => {
  ...
  for (const symbol in positions) {
    ...
    log(`♻️ Resumed position for ${symbol}: Qty ${pos.quantity}, Entry $${pos.entryPrice}`);
  }
};

log("🚀 Starting Trading Bot...");
log(`📋 Settings: Min Balance: $${MIN_BALANCE}, Trade Amount: $${TRADE_AMOUNT}, Profit Target: ${PROFIT_TARGET * 100}%`);
